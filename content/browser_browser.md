...
![浏览器内核和渲染进程各⾃职责](../imgs/浏览器内核和渲染进程各⾃职责.png)
...
- 对称加密
  ...
  ![使⽤对称加密实现 HTTPS](../imgs/使⽤对称加密实现%20HTTPS.png)
...
- ⾮对称加密
  ...
  ![⾮对称加密实现 HTTPS](../imgs/⾮对称加密实现%20HTTPS.png)
...
- 混合加密实现 HTTPS
  ...
  ![混合加密实现 HTTPS](../imgs/混合加密实现%20HTTPS.png)
...
- 添加数字证书
  ...
  ![完整的 HTTPS 请求流程](../imgs/完整的%20HTTPS%20请求流程.png)
  ...
  - 证书相关延伸阅读
        ...
        要讲数字证书链，就要了解我们的CA机构也是分两种类型的，中间 CA(Intermediates CAs) 和根 CA(Root CAs)，通常申请者都是向中间 CA 去申请证书的，⽽根 CA 作⽤就是给中间 CA 做认证，通常，⼀个根 CA 会认证很多中间的 CA，⽽这些中间 CA ⼜可以去认证其它的中间 CA。

        ⽐如你可以在Chrome上打开官⽹，然后点击地址栏前⾯的那把⼩锁，你就可以看到 `*.geekbang.org` 的证书是由中间 CA GeoTrust RSA CA2018 颁发的，⽽中间 CA GeoTrust RSACA2018 ⼜是由根 CA DigiCert Global Root CA 颁发的，所以这个证书链就是：`*.geekbang.org`--->`GeoTrust RSA CA2018`-->`DigiCert Global Root CA`。
